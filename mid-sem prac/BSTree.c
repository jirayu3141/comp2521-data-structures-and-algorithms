...
BSTree newBSTree(void) { return NULL; }
...
// display a BSTree
void showBSTree(BSTree t) { doShowBSTree(t); }
...
void BSTreeLevelOrder(BSTree t) {
    ...
    while (!QueueIsEmpty(newQ)) {
    	Item curr = QueueLeave(newQ);
        printf("%i ", curr->value);
        if (curr->left != NULL) QueueJoin(newQ, curr->left);
        if (curr->right != NULL) QueueJoin(newQ, curr->right);
    }
    dropQueue(newQ);
