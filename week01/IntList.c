...
// IntListInsertInOrder wrtitten by Jirayu Sirivorawong (z5125805) 7 Jun 2019
...
{
	//if the list is empty, insert it at the end
	if(L->first == NULL) {
		IntListInsert(L, v);
	} else {
		//insert infront for smallest number
		if(v < L->first->data){
			struct IntListNode *new	= newIntListNode(v);
			new->next = L->first;
			L->first = new;
			L->size++;
		//insert at the end for largest number
		} else if (v > L->last->data) {
			IntListInsert(L,v);
		//insert inbetweem 2 nodes
		} else {
			...
			while(curr->next!= NULL){
				if(curr->data < v && curr->next->data >= v){
					//create new node to store the current value
					struct IntListNode *new	= newIntListNode(v); 
					//store the address of the next node to the newly created node
					new->next = curr->next; 
					curr->next = new; //point the current node to the newly created node
					L->size++; //adjust the size of IntList
				}
				curr = curr->next;
			}
		}
