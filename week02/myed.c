...
int main (int argc, char *argv[])
{
	...
	char fname[MAX];	//stirng to store the name of the file
	strcpy (fname, argv[1]);	//put the name into fname array

	FILE *f;	//create file pointer
	//check for error
	if ((f = fopen (fname, "r")) == NULL)
		err (EX_IOERR, "couldn't open '%s'", fname);
	DLList lines = getDLList (f); //create a LL and from text file
	fclose (f);
